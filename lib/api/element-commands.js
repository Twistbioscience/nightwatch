    ...
    var el = Protocol.element(using, value, function(result) {
      if (result.status !== 0) {
        ...
        args.push(function(r) {
          if (callback.call(client.api, r)) {
            Logger.info('*** callback handled any encountered errors');
          } else {
            if (r.status !== 0) {
              Logger.info('*** detected an error');

              var errorMessage = `ERROR status: ${r.errorStatus} details: ${JSON.stringify(r.value)}`;
